...
class livox_model():
    ...
    def placeholder_inputs(self, batch_size):
        ...
        input_bev_img_pl = tf.placeholder(tf.bool,
                                          shape=(batch_size, self.img_height, self.img_width, 30))

        label_gt_total_channels = tf.placeholder(tf.float32,
                                                 shape=(batch_size, int(self.img_height/8), int(self.img_width/8), 15))

        return input_bev_img_pl, label_gt_total_channels
    ...
    def livox_detection(self, input_bev_img_pl, end_points):
        with slim.arg_scope([slim.conv2d, slim.fully_connected],
            ...
            with slim.arg_scope([slim.conv2d], padding='SAME',
                ...
                feature_out = slim.conv2d(feature_out, 14, [3, 3],
                                          normalizer_fn=None, activation_fn=None)
                end_points['feature_out'] = feature_out
        return end_points

    def get_model(self, input_bev_img_pl, label_gt_total_channels):
        end_points = {}
